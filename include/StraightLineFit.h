...
const int nstripmx=64;

class StraightLineFit {
 public :
  StraightLineFit();
  StraightLineFit(bool type, double* xv, double* yv, double* ye, bool* used, int occu, int occu2, int first, int last, float mxdev);
 ...
 private :
  ...
  bool isPosition;   // Postion fit or time fit
  ...
  int reject_count; //to reject the tracks are having hits above 32 strip
};
#endif
